...
int g_tunsrc_set = 1;
struct naas_vppapi_client *g_vppapicli;
...
lcp_nl_handle_seg6_local(int is_add, struct rtnl_route *route, struct rtnl_nexthop *nh)
{
	...
	case SEG6_LOCAL_ACTION_END_DT4:
		...
		assert(behavior >= 0);
		naas_vppapi_sr_localsid_add_del(g_vppapicli, is_add, behavior,
				nl_addr_get_binary_addr(dst), table_id);
		break;
...
lcp_tunsrc_set()
{
	...
	naas_vppapi_set_sr_encaps_source_addr(g_vppapicli, &src);
}
...
lcp_nl_handle_seg6(int is_add, struct rtnl_route *route, struct rtnl_nexthop *nh)
{
	...
	if (is_add) {
		lcp_tunsrc_set();
		naas_vppapi_sr_policy_add(g_vppapicli, bsid, srh->segments, srh->first_segment);
	}

	naas_vppapi_sr_steering_add_del(g_vppapicli, is_add, phy_sw_if_index,
			family, prefix, prefixlen, table_id, bsid);

	if (!is_add) {
		naas_vppapi_sr_policy_del(g_vppapicli, bsid);
	}
}
	...
	case RTM_NEWLINK:
		...
		if (rtnl_link_is_vrf(link)) {
			if (!rtnl_link_vrf_get_tableid(link, &tableid)) {
				naas_vppapi_ip_table_add_del(g_vppapicli, is_add, 0, tableid);
				naas_vppapi_ip_table_add_del(g_vppapicli, is_add, 1, tableid);
			} else {
...
{
	struct nl_sock *sock;
	int rc, fd, opt, dflag, long_option_index, log_level;
	int pid_file_fd;
	...
	dflag = 0;
	while ((opt = getopt_long(argc, argv, "hvdl:", long_options, &long_option_index)) != -1) {
		switch (opt) {
		case 0:
	...
	rc = naas_vppapi_vac_init(&vac, DAEMON_NAME);
	if (rc < 0) {
		naas_logf(LOG_ERR, -rc, "Failed to initalize VAC");
		naas_pid_file_close(pid_file_fd, DAEMON_NAME);
		return EXIT_FAILURE;
	}
	g_vppapicli = &vac.vac_base;

	naas_dlist_init(&g_lcp_itf_pair_head);	
	naas_vppapi_lcp_itf_pair_get(&vac, lcp_itf_pair_get_handler, &g_lcp_itf_pair_head);
	...
	naas_vppapi_deinit(g_vppapicli);
	naas_pid_file_close(pid_file_fd, DAEMON_NAME);
	return 0;
}
